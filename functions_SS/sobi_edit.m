...
if (size(X,1)<size(X,2))
    ...
    % X=mult*X;
    [puiss,k]	= sort(diag(S))	;
    rangeW		= m-n+1:m			; % indices to the m  most significant directions
    scales		= sqrt(puiss(rangeW))		; % scales
     WW  		= diag(1./scales)  * VV(1:m,k(rangeW))'	;	% whitener
     iWW  		= VV(1:m,k(rangeW)) * diag(scales) 	;	% its pseudo-inverse
    X		= WW*X;  
else
    [UU,S] 		= eig((X*X')/size(X,2))	; 
    [puiss,k]	= sort(diag(S))	;
    rangeW		= m-n+1:m			; % indices to the m  most significant directions
    scales		= sqrt(puiss(rangeW))		; % scales
    WW  		= diag(1./scales)  * UU(1:m,k(rangeW))'	;	% whitener
    iWW  		= UU(1:m,k(rangeW)) * diag(scales) 	;	% its pseudo-inverse
    X		= WW*X; 
end
